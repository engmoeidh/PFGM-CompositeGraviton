...
It is worth stressing that the constant--gradient background
\(q_\mu \neq 0\) breaks exact Lorentz invariance, since it selects a preferred
timelike direction.  The spin decomposition above is therefore performed with
respect to the background metric \(\bar g_{\mu\nu}\) and the four--momentum
\(k_\mu\) of fluctuations, rather than in a fully Lorentz--invariant vacuum.
In the induced--gravity regime of interest the dynamics of long--wavelength
modes are nevertheless well described by an approximately
Lorentz/diffeomorphism--invariant effective field theory for the composite
metric, so that the use of the standard projector basis and the associated
tensor structures is appropriate for extracting the physical tensor and scalar
sectors of the composite correlator.
...
pole with positive residue. In the overlap between the microscopic description and the induced
Einstein--Hilbert effective theory, the residue \(Z_2\) can be matched directly
to that of the linearised Einstein propagator, implying a scaling
\(Z_2 \sim 1/M_{\rm Pl}^2\) up to convention--dependent numerical factors; see
\cref{app:induced_implications} for a more detailed discussion of this
identification.
...
\subsection{Static limit and Newtonian potential}
\label{subsec:lin_Newtonian_potential}

It is instructive to check explicitly that the composite graviton reproduces
the Newtonian potential between nonrelativistic sources.  Consider two static,
conserved sources \(T^{(1)}_{\mu\nu}\) and \(T^{(2)}_{\mu\nu}\) with masses
\(m_1\) and \(m_2\), localised in space and at rest in the constant--gradient
background, so that \(T^{(a)}_{00} = \rho_a(\mathbf{x})\), \(\rho_a \simeq m_a
\delta^{(3)}(\mathbf{x}-\mathbf{x}_a)\), and spatial components are suppressed
by powers of the velocities.  In the induced--gravity regime the transverse--traceless
composite graviton propagator takes the form
\begin{equation}
  \big\langle h_{\mu\nu}(k)\,h_{\rho\sigma}(-k)\big\rangle_{\rm TT}
  \simeq \frac{Z_2}{k^2 + \mathrm{i}\epsilon}\,
    \mathcal{P}^{(2)}_{\mu\nu\rho\sigma},
  \label{eq:lin_TT_prop_again}
\end{equation}
with \(Z_2>0\) and the spin--2 projector \(\mathcal{P}^{(2)}_{\mu\nu\rho\sigma}\)
constructed from the background metric, as in
\cref{app:conventions_projectors,app:hh_correlator}.

The tree--level exchange amplitude between the two sources mediated by the
composite graviton is, schematically,
\begin{equation}
  \mathcal{A}(k)
  \sim T^{(1)\,\mu\nu}(k)\,
       \big\langle h_{\mu\nu}(k)\,h_{\rho\sigma}(-k)\big\rangle_{\rm TT}\,
       T^{(2)\,\rho\sigma}(-k),
\end{equation}
so that in the nonrelativistic, static limit \(k^0\to 0\) and
\(T_{00}\gg T_{0i},T_{ij}\), one finds
\begin{equation}
  \mathcal{A}(k^0=0,\mathbf{k})
  \propto Z_2\,\frac{\tilde\rho_1(\mathbf{k})\,\tilde\rho_2(-\mathbf{k})}
                   {\mathbf{k}^2},
\end{equation}
up to conventional numerical factors.\footnote{A careful derivation using the
standard linearised Einstein equations and the coupling \(h^{\mu\nu}T_{\mu\nu}\)
can be found in any textbook on general relativity; see, for example,
Refs.~\cite{Weinberg1995,Ryder2009}.  Here we simply note that the structure of
the composite propagator and coupling is identical to that of the linearised
Einstein graviton in the induced--gravity regime, with \(Z_2\) identified with
\(1/M_{\rm Pl}^2\) as discussed in \cref{app:induced_implications}.}
Fourier transforming to position space yields the familiar Newtonian potential
between the two sources,
\begin{equation}
  V(r)
  \simeq -\,\frac{G_{\rm eff} m_1 m_2}{r},
  \qquad
  G_{\rm eff} \sim \frac{1}{8\pi M_{\rm Pl}^2},
\end{equation}
with the sign fixed by the positivity of \(Z_2\) and the tensor structure of
\(\mathcal{P}^{(2)}_{\mu\nu\rho\sigma}\).  In the healthy band, the scalar
sector encoded in \(C(k^2)\) and \(D(k^2)\) is massive and does not generate an
additional long--range \(1/r\) force, so that the leading static potential is
purely of Newtonian form in the induced--gravity window.
...
\begin{equation}
  ...
  \simeq -\,\frac{G_{\rm eff} m_1 m_2}{r},
  \qquad
  G_{\rm eff} \simeq \frac{1}{8\pi M_{\rm Pl}^2},
\end{equation}
where the precise numerical coefficient relating \(G_{\rm eff}\) and
\(M_{\rm Pl}^{-2}\) follows from the standard general--relativistic computation
in the linearised Einstein theory.\footnote{See, for example,
Refs.~\cite{Weinberg1995,Ryder1996} for the derivation of the Newtonian limit
of general relativity.  In our setup the composite graviton has the same
transverse--traceless propagator and coupling structure, with
\(Z_2 \simeq 1/M_{\rm Pl}^2\) as discussed in \cref{app:induced_implications},
so the standard relation between \(G\) and \(M_{\rm Pl}\) is inherited in the
induced--gravity regime.}

This check shows explicitly that the composite graviton mediates an attractive
\(1/r\) potential between nonrelativistic masses with the same tensor structure
and sign as in general relativity, and that the effective Newton constant is
set by the induced Planck mass.  In the overlap regime where the induced
Einstein--Hilbert term reproduces the observed value of Newton's constant,
i.e. for parameter choices that match \(M_{\rm Pl}\) to the measured Planck
scale \cite{Hanash:PFGM-I,Hanash:Induced-EH}, one has
\(G_{\rm eff}\simeq G_{\rm N}\).  The scalar sector encoded in
\(C(k^2)\) and \(D(k^2)\) is massive and does not generate an additional
long--range \(1/r\) force in the healthy band, so the leading static potential
remains purely Newtonian in the infrared.
...
nonlocality of the emergent metric field. More explicitly, in four spacetime dimensions a symmetric tensor \(h_{\mu\nu}\)
has ten independent components.  Linearised diffeomorphism invariance,
\(h_{\mu\nu} \to h_{\mu\nu} + \partial_\mu \xi_\nu + \partial_\nu \xi_\mu\),
allows one to use the four gauge functions \(\xi_\mu\) to eliminate four
combinations of components, while the linearised Einstein equations and the
Bianchi identity imply four constraint equations that remove four further
combinations.  The number of independent propagating degrees of freedom in the
tensor sector is therefore
\begin{equation}
  N_{\rm DOF} = 10 - 4 - 4 = 2,
\end{equation}
corresponding to the two transverse--traceless graviton polarisations familiar
...
\subsection{Weinberg--Witten considerations}
\label{subsec:WW}

The appearance of a massless spin--2 sector in the infrared effective theory
of the composite metric \(g^{\rm eff}_{\mu\nu}\), starting from a microscopic
theory with only a scalar field on Minkowski space, raises natural questions
about the applicability of the Weinberg--Witten theorem.  In its standard form,
the theorem states that in a Poincar\'e--invariant quantum field theory on
\((\mathbb{R}^{1,3},\eta_{\mu\nu})\) with a conserved, Lorentz--covariant
stress tensor \(T_{\mu\nu}(\eta)\) that generates time translations, no
massless particle of spin \(s>1\) can carry the corresponding
energy--momentum charge \cite{WeinbergWitten1980}.

In the present construction the hypotheses of the theorem are not jointly
satisfied in the regime where the induced gravitational sector is relevant.
First, once the composite metric \(g^{\rm eff}_{\mu\nu}[\Phi]\) acquires an
induced Einstein--Hilbert term, the appropriate long--wavelength description is
a diffeomorphism--invariant effective field theory on the curved background
\((\mathcal M,g^{\rm eff}_{\mu\nu}[\Phi_0])\), rather than a globally
Poincar\'e--invariant theory on \((\mathbb{R}^{1,3},\eta_{\mu\nu})\).  Generic
backgrounds \(\Phi_0\) with nonzero gradient break Minkowski Poincar\'e
invariance.  Second, as in general relativity, the graviton in this induced
EFT does not admit a local, gauge--invariant, Lorentz--covariant stress tensor:
the spin--2 mode couples to the diffeomorphism--covariant Hilbert tensor
\(T_{\mu\nu}(g^{\rm eff})\) of the induced action, not to a conserved
\(T_{\mu\nu}(\eta)\) on the original Minkowski background.  Third, we do not
construct Minkowski one--particle graviton states that carry the microscopic
\(T_{\mu\nu}(\eta)\) charge; the massless spin--2 excitations studied here are
collective infrared degrees of freedom of the induced metric EFT.

Our claim is therefore modest and precise: in the induced--gravity window the
background--field EFT for \(g^{\rm eff}_{\mu\nu}\) reproduces the phenomenology
of general relativity with \(G\to G^{(\\Phi)}_{\rm ind}\) as discussed in
Ref.~\cite{Hanash:Induced-EH}, while remaining consistent with the
Weinberg--Witten theorem because the latter's assumptions (exact Minkowski
Poincar\'e invariance, a Lorentz--covariant conserved stress tensor carrying
the graviton charge, and an \(S\)-matrix built from Minkowski one--particle
spin--2 states) are not simultaneously realised in the regime of interest.\footnote{A
useful analogy is provided by emergent excitations in condensed--matter
systems: the microscopic theory may be formulated on a fixed background, yet
the relevant low--energy quasiparticles propagate on an effective medium and
carry charges associated with its conserved quantities, without violating the
Weinberg--Witten assumptions.}
...
The emergent nature of the metric and graviton raises interesting
...
Finally, an important conceptual point is that the emergence of a healthy massless
spin--2 composite in PFGM is not generic for arbitrary scalar composites.
For a single scalar field \(\Phi\), most local composite tensors one can
construct will either fail to produce a transverse--traceless pole with a
positive residue or will couple nonuniversally to matter in a way that is
incompatible with an induced Einstein--Hilbert sector.  In the present
framework both the special rank--one disformal form
\begin{equation}
  g^{\rm eff}_{\mu\nu}[\Phi]
  = \eta_{\mu\nu} + \alpha\,\partial_\mu\Phi\,\partial_\nu\Phi
\end{equation}
and the existence of an induced Einstein--Hilbert action for
\(g^{\rm eff}_{\mu\nu}\) are crucial: together they ensure that the composite
tensor sector contains a massless spin--2 pole with the correct projector
structure, residue, and universal coupling to the Hilbert stress tensor of the
induced EFT.  It would be interesting to investigate whether, under mild
assumptions (single scalar proto--field, derivative composite metric, and an
induced GR--like effective action), the PFGM disformal structure is essentially
unique in admitting a healthy composite graviton, or whether closely related
...
\label{subsec:outlook_GW}

The composite graviton constructed in this work provides a microscopic origin
for the tensor modes used in previous post--Newtonian and strong--field
analyses of PFGM.  In the induced--gravity regime the transverse--traceless
sector of the composite metric fluctuation obeys the same linearised wave
equation and couples to matter with the same structure as the graviton in
general relativity, up to corrections suppressed by curvature--squared
operators and higher--derivative terms.  At leading order one therefore
expects the gravitational--wave sector of PFGM to exhibit a propagation speed
\(c_T\simeq 1\), two observable tensor polarisations, and no additional
long--range scalar dipole channel in the healthy band, in agreement with the
set--up adopted in PFGM~II and III.

Deviations from the general--relativistic gravitational--wave phenomenology
are expected to enter through higher--derivative corrections in the induced
gravitational action and through mesoscopic structure associated with the
proto--field coherence scale.  A dedicated study of waveform phasing,
polarisation content, and possible mesoscopic imprints on gravitational--wave
signals, building on the composite graviton EFT developed here, is left for
future work.
...
\subsection{Benchmark example in the healthy band}
\label{app:benchmark_healthy_band}

For concreteness it is useful to exhibit a simple benchmark point in the
healthy band.  Consider the toy choice
\begin{equation}
  P(X) = -\frac12 X + \frac{\lambda}{4} X^2,
\end{equation}
with a background value \(X_0 = 1\) and parameter \(\lambda = 2\).  The
derivatives of \(P\) at the background are
\begin{equation}
  P'(X_0) = -\frac12 + \frac{\lambda}{2} X_0
  = -\frac12 + 1 = 0.5,
  \qquad
  P''(X_0) = \frac{\lambda}{2} = 1.0.
\end{equation}
In terms of the notation of \cref{app:quad_action}, we therefore have
\begin{equation}
  Z_s = P'(X_0) = 0.5,
  \qquad
  Z_t = P'(X_0) + 2 X_0 P''(X_0)
      = 0.5 + 2\cdot 1 \cdot 1 = 2.5,
\end{equation}
and the corresponding sound speed is
\begin{equation}
  c_s^2 = \frac{Z_s}{Z_t} = \frac{0.5}{2.5} = 0.2.
\end{equation}
These values clearly satisfy the healthy--band inequalities
\begin{equation}
  Z_t > 0,
  \qquad
  Z_s > 0,
  ...
  c_s^2 > 0,
\end{equation}
and \(c_s^2<1\) ensures subluminal propagation at low momenta.  A quartic
derivative coupling \(\beta_2\) of order unity and a cutoff scale \(\Lambda\)
can then be chosen such that the higher--derivative terms remain perturbative
for the momenta of interest, as discussed in \cref{app:quad_dispersion}.
This benchmark point lies within the numerically determined healthy band in the
\((P'(X_0),P''(X_0))\) plane shown in
\cref{fig:healthy_band_scan,tab:healthy_band_stats}, and provides a concrete
example of a background around which the proto--field fluctuations are stable
and well described by the induced--gravity effective theory.

\paragraph{}
In units where the induced Planck mass is set to \(M_{\rm Pl}=1\), the matching
in \cref{app:induced_implications} then implies a spin--2 residue \(Z_2\) of
order unity for this benchmark choice.
...
sign.
\paragraph{}
This conclusion is supported both analytically and numerically.  Analytically,
one can expand the projector contraction in simple limits such as
\(|\mathbf{k}|\ll |q_0|\), where the leading contribution to \(\Pi_2(k^2)\) is
manifestly positive, and we have also evaluated \(\Pi_2(k^2)\) explicitly using
a computer algebra system for generic timelike \(q_\mu\) and momenta lying in
the healthy band, finding it to be strictly positive in all cases.  The sample
values of the spin--2 contraction \(F_2(q,k)\) stored in
\texttt{data/spin2\_F2\_samples.csv} and summarised in
\cref{fig:spin2_F2_vs_k2,tab:spin2_F2_stats} provide a numerical illustration
of this positivity: all entries have \(F_2>0\), with no sign changes in the
region of interest.
\newline
...
In the present context the constant--gradient background and induced--gravity
effective description mean that exact microscopic Lorentz invariance is
broken.  The K\"all\'en--Lehmann representation is therefore used here as a
formal tool for encoding the spin--2 and spin--0 spectral content of the
composite correlator in the approximately Lorentz/diffeomorphism--invariant
infrared effective field theory defined by the induced Einstein--Hilbert term.
