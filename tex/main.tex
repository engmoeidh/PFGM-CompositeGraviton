...
As already noted in \cref{subsec:setup_action}, we assume that the microscopic
parameters of the proto--field theory are chosen such that there exists a
healthy band of backgrounds for which small fluctuations are free of ghosts and
gradient instabilities.  On the constant--gradient background this amounts to
requiring that the quadratic action for fluctuations has a positive--definite
kinetic term and a hyperbolic principal symbol with respect to the composite
metric \(\bar g_{\mu\nu}\).
...
\end{equation}
where \(\mathcal{D}\) is a differential operator whose leading, second--order part has principal
symbol is controlled by an effective characteristic metric constructed from
...
develop a massless ghost pole in the infrared.
...
Lorentzian principal symbol, one finds that the transverse--traceless sector is
governed by a coefficient of the form
\begin{equation}
  A(k^2)
...
appear in the projector decomposition
\eqref{eq:hh_projector_decomp}.  The starting point is the linear relation
between the composite metric fluctuation and the proto--field fluctuation,
together with the Gaussian propagator for \(\chi\) obtained from the quadratic
action in \cref{app:quad_action}.
...
Using these relations, the composite metric two--point function at leading
order is obtained by a single Wick contraction,
\begin{align}
  \big\langle h_{\mu\nu}(k)\,h_{\rho\sigma}(-k)\big\rangle
  &= -\alpha^2
     \big(q_\mu k_\nu + q_\nu k_\mu\big)
  ...
  \label{eq:app_hh_raw}
\end{align}
which coincides with \eqref{eq:hh_correlator_raw} in the main text.  The tensor
structure in the numerator can be regarded as an element of the space of
symmetric rank--2 tensors in the \((\mu\nu)\) and \((\rho\sigma)\) index pairs,
constructed from the available vectors \(q_\mu\), \(k_\mu\), and the background
metric \(\bar g_{\mu\nu}\).

To analyse its spin content it is convenient to work in the timelike--gradient
frame
\begin{equation}
  q_\mu = (q,0,0,0),
\end{equation}
so that rotational symmetry acts in the usual way on the spatial components of
\(k_\mu\).  In this frame one can decompose the tensor
\((q_\mu k_\nu + q_\nu k_\mu)\) into pieces that transform irreducibly under
spatial rotations, separating scalar, vector, and tensor contributions.  The
resulting structure can then be mapped directly onto the set of projectors
\(\mathcal{P}^{(2)}\), \(\mathcal{P}^{(1)}\),
\(\mathcal{P}^{(0\text{-s})}\), and \(\mathcal{P}^{(0\text{-w})}\) introduced
in \cref{app:conventions_projectors}.
...
\begin{equation}
  ...
  + D(k^2)\,\mathcal{P}^{(0\text{-w})}_{\mu\nu\rho\sigma},
\end{equation}
where the coefficient functions \(A,B,C,D\) can be obtained by contracting with
the corresponding projectors.  Making use of the completeness and
orthogonality relations summarised in \cref{app:conv_proj_algebra}, one finds
for example
\begin{align}
  A(k^2)
  &= \frac{1}{\mathcal{N}_2}\,
...
algebra.  Inserting the explicit expression \eqref{eq:app_hh_raw} into these
formulas yields closed expressions for \(A,B,C,D\) in terms of the background
data and the microscopic parameters of the proto--field theory.

In the timelike--gradient frame \(q_\mu=(q,0,0,0)\) the coefficient functions
simplify considerably.  Writing \(k^\mu=(\omega,\mathbf{k})\),
\(k^2=-\omega^2+\mathbf{k}^2\), and using the fact that the characteristic
tensor \(K^{\mu\nu}\) can be brought to a form proportional to \(\bar g^{\mu\nu}\)
at leading order in the infrared, one finds that the transverse--traceless
...
\begin{equation}
  ...
  + \text{subleading},
\end{equation}
with a positive residue \(Z_2>0\) and an effective mass parameter \(m_2^2\)
whose vanishing in the induced--gravity regime is discussed in
\cref{subsec:spin2_massless_pole}.  The scalar coefficients \(C(k^2)\) and
\(D(k^2)\) take the schematic form
\begin{equation}
  C(k^2), D(k^2)
  = \frac{Z_0}{k^2 - m_0^2 + \mathrm{i}\epsilon}
  + \text{subleading},
\end{equation}
with \(Z_0>0\) and \(m_0^2\) generically nonzero in the healthy band, as
emphasised in \cref{subsec:spin2_scalar_sector}.  These expressions provide the
explicit realisation of the spin decomposition used in the main text and serve
as the starting point for the spectral analysis in \cref{app:spectral}.
...
dimensionful coefficients of curvature--squared operators, and the ellipsis
